...
export default function PaymentPage() {
    ...
    const { selectedSeats, showInfo, totalAmount, setShow, clearSeats } = useBookingStore();
    ...
    const totalConvenience = selectedSeats.length * CONVENIENCE_FEE_PER_TICKET;
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            toast.error('Payment Failed: ' + (error.message || 'Error occurred'));
            setIsProcessing(false);
        }
    };

    const onSubmitCard = (data) => {
        if (data.cardNumber === '4111 1111 1111 1111' || activeTab === 'Card') {
            handlePayment();
        } else {
            toast.error("Test card failed");
        }
    };
            ...
            <div className="max-w-6xl mx-auto px-4 flex flex-col lg:flex-row gap-8">
                    ...
                    <div className="flex bg-gray-800 p-1 rounded-lg mb-8 overflow-x-auto hide-scrollbar">
                        {['Card', 'UPI', 'Net Banking', 'Wallet'].map(tab => (
                            <button
                                ...
                                className={`flex-1 min-w-[100px] text-sm font-medium py-2 px-4 rounded-md transition-colors ${activeTab === tab ? 'bg-primary-500 text-white shadow-md' : 'text-gray-400 hover:text-white'
                                    }`}
                            >
                                {tab}
                            </button>
                        ...
                        {activeTab === 'Card' && (
                            <form onSubmit={handleSubmit(onSubmitCard)} className="space-y-5">
                                ...
                                <button
                                    ...
                                    className="w-full py-4 mt-6 bg-primary-500 hover:bg-primary-600 rounded-lg text-white font-bold transition-all disabled:opacity-75 disabled:cursor-wait flex justify-center items-center gap-2 shadow-[0_0_15px_rgba(229,9,20,0.3)]"
                                >
                                    {isProcessing ? (
                            ...
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        />
                                        <button className="bg-gray-700 hover:bg-gray-600 text-white px-4 py-2 rounded-lg font-medium transition-colors">Verify</button>
                                    </div>
                                </div>
                                <button
                ...
                <div className="w-full lg:w-[400px]">
                    <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden sticky top-24 shadow-2xl">
                        <div className="p-6">
                            <h3 className="text-lg font-bold text-white mb-4 uppercase tracking-wider text-sm border-b border-gray-800 pb-2">Booking Summary</h3>
                            ...
                            <div className="flex gap-4 items-start pb-6 border-b border-gray-800 mb-6">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-primary-500 mt-2 font-medium">{showInfo.date} • {showInfo.time}</p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-3 text-sm">
                                ...
                                <div className="flex justify-between text-gray-300">
                                    ...
                                    <span className="font-semibold text-white">₹{totalConvenience}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-primary-500/10 p-6 border-t border-primary-500/20">
                            <div className="flex justify-between items-center text-lg font-bold text-white">
                                ...
                                <span className="text-primary-500 text-2xl tracking-tight">₹{grandTotal}</span>
                            </div>
                        </div>
                    </div>
                </div>
